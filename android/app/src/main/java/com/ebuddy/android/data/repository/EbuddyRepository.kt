...
class EbuddyRepository(private val api:EbuddyApi, private val auth:AuthInterceptor, private val store:SessionStore, private val dao:MessageDao, private val stomp:StompClient) {
    ...
    suspend fun restore():Result<Session?> = runCatching { store.read()?.let { stored -> val s=Session(stored.token,stored.expiresAt,User(stored.user.id,stored.user.username,stored.user.displayName)); auth.setToken(s.token); _session.value=s; stomp.connect(); s } }
    suspend fun login(username:String,password:String):Result<Session> = runCatching { val r=api.login(LoginRequest(username,password)); persist(r); val s=Session(r.token,r.expiresAt,User(r.user.id,r.user.username,r.user.displayName)); _session.value=s; stomp.connect();s }.onFailure{_error.tryEmit(message(it))}
    suspend fun register(username:String,password:String,displayName:String):Result<Session> = runCatching { api.register(RegisterRequest(username,password,displayName)); login(username,password).getOrThrow() }.onFailure{_error.tryEmit(message(it))}
    ...
    private suspend fun persist(r:LoginResponse)=store.save(r)
