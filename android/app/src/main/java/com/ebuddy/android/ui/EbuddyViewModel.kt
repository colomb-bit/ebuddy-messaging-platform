...
class EbuddyViewModel(private val repo:EbuddyRepository):ViewModel(){
    ...
    init{viewModelScope.launch{repo.session.collect{session->_state.update{old->old.copy(session=session)}}};viewModelScope.launch{repo.errors.collect{e->_state.update{old->old.copy(error=e,loading=false,syncing=false)}}}}
