...
public final class EbuddyMidlet extends MIDlet implements NetworkListener {
    ...
    void login(String u,String p){login.setBusy(true);net.request(NetworkWorker.LOGIN,"/api/j2me/login",jsonLogin(u,p,"j2me"));}
    void register(String u,String p,String name){register.setBusy(true);net.request(NetworkWorker.REGISTER,"/api/j2me/register","{\"username\":\""+MiniJsonEscape.clean(u)+"\",\"password\":\""+MiniJsonEscape.clean(p)+"\",\"display_name\":\""+MiniJsonEscape.clean(name)+"\"}");}
    private String jsonLogin(String u,String p,String t){return "{\"username\":\""+MiniJsonEscape.clean(u)+"\",\"password\":\""+MiniJsonEscape.clean(p)+"\",\"client_type\":\""+t+"\"}";}
    ...
    private void handleNetworkResult(int op,String data,Exception error){if(error!=null){if(op==NetworkWorker.LOGIN&&login!=null)login.fail("Network unavailable");else if(op==NetworkWorker.REGISTER&&register!=null)register.fail("Network unavailable");else if(chat!=null)chat.fail("Retry later");return;}if(op==NetworkWorker.LOGIN){session=new Session();session.token=MiniJson.value(data,"token");session.expiresAt=MiniJson.number(data,"expires_at",0);session.userId=MiniJson.value(data,"id");if(session.token==null){login.fail("Invalid login response");return;}Store.saveSession(session);showContacts();}else if(op==NetworkWorker.REGISTER){showLogin();}else if(op==NetworkWorker.CONTACTS){if(contacts!=null)contacts.setItems(parseContacts(data));}else if(op==NetworkWorker.POLL){Vector v=parseMessages(data);if(chat!=null)chat.addMessages(v);session.cursor=MiniJson.number(data,"next_cursor",session.cursor);Store.saveSession(session);if(!paused)poll();}else if(op==NetworkWorker.SEND){if(chat!=null)chat.sent();}}
    private Vector parseContacts(String data){Vector out=new Vector();Vector a=MiniJson.splitObjects(data);for(int i=0;i<a.size();i++){String x=(String)a.elementAt(i);String u=MiniJson.value(x,"username");String n=MiniJson.value(x,"display_name");String id=MiniJson.value(x,"id");String s=MiniJson.value(x,"state");if(id!=null)out.addElement(new Contact(id,u,n==null?u:n,s));}return out;}
    private Vector parseMessages(String data){Vector out=new Vector();Vector a=MiniJson.splitObjects(data);for(int i=0;i<a.size();i++){String x=(String)a.elementAt(i);ChatMessage m=new ChatMessage();m.id=MiniJson.value(x,"id");m.from=MiniJson.value(x,"from_user_id");m.to=MiniJson.value(x,"to_user_id");m.body=MiniJson.value(x,"body");m.status=MiniJson.value(x,"status");m.sequence=MiniJson.number(x,"sequence",0);if(m.body!=null)out.addElement(m);}return out;}
}
